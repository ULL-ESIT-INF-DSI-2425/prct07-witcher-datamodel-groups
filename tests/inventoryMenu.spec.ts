...
describe("startInterface", () => {
  ...
  test.each([
    ...
    ["Descripción", "afilada", "103", "Daga Oscura", "Una daga muy afilada", "Madera"],
    ["Material", "madera", "104", "Bastón Mágico", "Hecho con magia ancestral", "Madera"],
  ])(
    "debe encontrar bienes por %s",
    async (criterioBusqueda, terminoBusqueda, id, name, description, material) => {
      // Mock de `getStock()` para que devuelva un bien coincidente
      inventario.getStock = vi.fn().mockReturnValue([
        {
      ...
      // Simular respuestas del usuario en el prompt
      mockPrompt
        .mockResolvedValueOnce({ option: "📦 Buscar bien" })
      ...
      // Verifica que el bien correcto fue impreso
  ...
  test("debe generar un informe de bienes más vendidos", async () => {
    ...
    const customer = new Customer("100",
      "Cliente1", "Raza1", "Ubicación1");
    inventario.addCustomer(customer);

    const transaction = new Transaction(
      "1", new Date("2023-01-01"),
      [{ item, quantity: 5 }], 500, customer, "sale",
    );
  ...
  test("debe mostrar el historial si hay transacciones para el participante", async () => {
    const participante = new Customer("p1", "Geralt de Rivia", "Humano", "Kaer Morhen");
    const item = new Item("1", "Espada de Plata", "Letal contra monstruos", "Plata", 3, 500);
  
    // Agregamos stock al inventario para permitir la venta
    inventario.addItem(item, 5);
    ...
    mockPrompt
      .mockResolvedValueOnce({ option: "🧾 Historial de transacciones por participante" })
    ...
    expect(output).toContain("Historial de transacciones para el participante con ID p1");
  ...
  test("debe mostrar mensaje si no hay transacciones para el participante", async () => {
    mockPrompt
      .mockResolvedValueOnce({ option: "🧾 Historial de transacciones por participante" })
      .mockResolvedValueOnce({ idParticipante2: "p999" }) // ID sin transacciones
      .mockResolvedValueOnce({ option: "❌ Salir" });
    ...
    expect(output).toContain("No hay transacciones registradas para ese participante.");
  });
  ...
  test("debe mostrar todas las transacciones cuando existen (🤝 Ver transacciones)", async () => {
    ...
    const item = new Item("i1", "Daga élfica", "Corta y precisa", "Acero", 1, 200);
  ...
  test("debe eliminar transacción existente (🤝 Eliminar transacción)", async () => {
    ...
    const item = new Item("i2", "Poción de maná", "Recupera energía mágica", "Cristal", 0.5, 150);
    inventario.addItem(item, 3);
    const tx = new Transaction("txDel", new Date(), [{ item, quantity: 1 }], 150, participante, "sale");
  ...
  test("debe encontrar transacción por fecha (🤝 Buscar transacciones)", async () => {
    ...
    const item = new Item("i3", "Collar mágico", "Artefacto de protección", "Oro", 0.3, 300);
    inventario.addItem(item, 2);
    const fecha = new Date("2025-03-21T00:00:00");
    const tx = new Transaction("txFecha", fecha, [{ item, quantity: 1 }], 300, participante, "sale");
    transacciones.processTransaction(tx);
  ...
  test("debe encontrar transacción por participante (🤝 Buscar transacciones)", async () => {
    ...
    const item = new Item("i4", "Laúd encantado", "Perfecto para serenatas", "Madera", 1, 180);
    inventario.addItem(item, 2);
    const tx = new Transaction("txPart", new Date(), [{ item, quantity: 1 }], 180, participante, "sale");
    transacciones.processTransaction(tx);
  ...
  test("debe mostrar mensaje si no encuentra coincidencias (🤝 Buscar transacciones)", async () => {
    ...
    expect(logs).toContain("No se encontraron transacciones que coincidan con la búsqueda.");
  });  
  ...
  test("debe modificar un mercader existente (👤 Modificar mercader)", async () => {
    ...
    const mercaderModificado = inventario.getMerchants().find(m => m.id === "m2");
  ...
  test("debe ordenar mercaderes por nombre ascendente (👤 Ordenar mercaderes)", async () => {
    inventario.addMerchant({ id: "m3", name: "Bruno", type: "Comida", location: "Oxenfurt" });
    inventario.addMerchant({ id: "m4", name: "Alfred", type: "Comida", location: "Oxenfurt" });
  ...
  test("debe ordenar mercaderes por nombre descendente (👤 Ordenar mercaderes)", async () => {
    mockPrompt
      .mockResolvedValueOnce({ option: "👤 Ordenar mercaderes" })
      .mockResolvedValueOnce({ criterioMercader: "Nombre", ordenMercader: "Descendente" })
      .mockResolvedValueOnce({ option: "❌ Salir" });
  ...
  test("debe ordenar mercaderes por tipo ascendente (👤 Ordenar mercaderes)", async () => {
    inventario.addMerchant({ id: "m5", name: "Cedric", type: "Herramientas", location: "Skellige" });
    inventario.addMerchant({ id: "m6", name: "Derek", type: "Alquimia", location: "Skellige" });
  
    mockPrompt
      .mockResolvedValueOnce({ option: "👤 Ordenar mercaderes" })
      .mockResolvedValueOnce({ criterioMercader: "Tipo", ordenMercader: "Ascendente" })
      .mockResolvedValueOnce({ option: "❌ Salir" });
  ...
  test("debe ordenar mercaderes por tipo descendente (👤 Ordenar mercaderes)", async () => {
    mockPrompt
      .mockResolvedValueOnce({ option: "👤 Ordenar mercaderes" })
      .mockResolvedValueOnce({ criterioMercader: "Tipo", ordenMercader: "Descendente" })
      .mockResolvedValueOnce({ option: "❌ Salir" });
  ...
  test("debe mostrar mensaje si participante no existe (🤝 Registrar transacción)", async () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith("No se encontró un participante válido.");
  });
  
  test("debe registrar transacción de compra con ítem nuevo", async () => {
    inventario.addCustomer(new Customer("cli-1",
      "Geralt", "Raza1", "Kaer Morhen"));
  ...
  test("debe registrar transacción de compra con ítem existente", async () => {
    inventario.addCustomer(new Customer("cli-2",
      "Yennefer", "Raza1", "Vengerberg"));

    const item = new Item("i1", "Poción de maná", "Restablece energía mágica", "Vidrio", 0.5, 100);
    inventario.addItem(item, 10);
  ...
  test("debe registrar transacción de venta", async () => {
    ...
    const item = new Item("i2", "Cristal mágico", "Elemento arcano", "Cristal", 1, 300);
    inventario.addItem(item, 5);
  ...
  test("debe registrar transacción de devolución", async () => {
    ...
    const item = new Item("i3", "Escudo", "Protección básica", "Hierro", 5, 150);
    inventario.addItem(item, 3);
  ...
  test("debe mostrar mensaje si no se seleccionan ítems válidos", async () => {
    inventario.addCustomer(new Customer("cli-3",
      "Ciri", "Raza1", "Cintra"));
  ...
  test("debe mostrar mensaje si transacción no se procesa", async () => {
    ...
    const failProcess = vi.spyOn(transacciones, "processTransaction").mockReturnValue(false);
    inventario.addCustomer(new Customer("cli-4",
      "Eskel", "Raza1", "Kaer Morhen"));
    const item = new Item("i4", "Botas de cuero", "Botas reforzadas", "Cuero", 2, 200);
