  ...
  while (true) {
    ...
    switch (option) {
      ...
      case "🤝 Registrar transacción":
        ...
        const { idTransaccion, fechaTransaccion, idParticipante } =
          await inquirer.prompt([
            {
              type: "input",
              name: "idTransaccion",
        ...
        const
          totalAmount = itemsTransaccion.reduce(
            (total, { item, quantity }) => total + item.value * quantity,
            0,
          );

        const resultado = transacciones.processTransaction(
          participante,
          itemsTransaccion,
          tipoTransaccion === "Venta" ? "sale" : tipoTransaccion === "Compra" ? "purchase" : "return",
        );
