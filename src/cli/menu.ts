    ...
    switch (option) {
      case "📦 Añadir bien":
        ...
        if (
          ...
          console.log(
            `Se han añadido ${cantidadExtra} unidades al bien "${bienPorID.item.name}".`,
          );
        } else if (
          (bienPorID &&
          ...
          console.log(
            "Asegúrate de introducir un ID y nombre que coincidan con un bien existente si deseas añadir al stock, o elige un ID y nombre únicos si es un bien nuevo.\n",
          );
        } else {
      ...
      case "📦 Eliminar bien":
        ...
        if (bienAEliminar) {
          ...
          console.log(
            "No se ha encontrado ningún bien con ese ID en el inventario.",
          );
        }
        break;
      ...
      case "📦 Ver bienes":
        ...
        if (stock.length === 0) {
          ...
          stock.forEach((entry, index) => {
            console.log(
              `${index + 1}. ID: ${entry.item.id}, Nombre: ${entry.item.name}, Descripción: ${entry.item.description}, Material: ${entry.item.material}, Peso: ${entry.item.weight}, Valor: ${entry.item.value}, Cantidad: ${entry.quantity}`,
            );
          });
        }
        break;
      ...
      case "👤 Ver clientes":
        ...
        if (clientes.length === 0) {
          ...
          clientes.forEach((cliente, index) => {
            console.log(
              `${index + 1}. ID: ${cliente.id}, Nombre: ${cliente.name}, Raza: ${cliente.race}, Ubicación: ${cliente.location}`,
            );
          });
        }
        break;
      ...
      case "👤 Ver mercaderes":
        ...
        if (mercaderes.length === 0) {
          ...
          mercaderes.forEach((mercader, index) => {
            console.log(
              `${index + 1}. ID: ${mercader.id}, Nombre: ${mercader.name}, Tipo: ${mercader.type}, Ubicación: ${mercader.location}`,
            );
          });
        }
        break;
      ...
      case "🤝 Registrar transacción":
        ...
        while (true) {
          ...
          if (!agregarOtro) break;
        }

        const tipo = tipoTransaccion.toLowerCase() as
          | "purchase"
          | "sale"
          | "return";
        const exito = transacciones.processTransaction(
          participante,
          items,
          tipo,
        );
      ...
      case "🤝 Ver transacciones":
        ...
        if (historial.length === 0) {
          ...
          historial.forEach((tx, index) => {
            console.log(
              `${index + 1}. ID: ${tx.id}, Fecha: ${tx.date.toLocaleString()}, Tipo: ${tx.type}, Monto: ${tx.totalAmount} coronas, Participante: ${tx.participant}, Bienes: ${tx.items.map((item) => `${item.name})`).join(", ")}`,
            );
          });
        }
        break;
      ...
      case "🤝 Eliminar transacción":
        ...
        const eliminada = transacciones.removeTransaction(idTransaccion);
        if (eliminada) {
      ...
      case "🧾 Historial de transacciones por participante":
        ...
        const historialParticipante =
          transacciones.getTransactionsHistoryByParticipant(idParticipante);
        if (historialParticipante.length === 0) {
          ...
          console.log(
            `Historial de transacciones para el participante con ID ${idParticipante}:`,
          );
          historialParticipante.forEach((tx, index) => {
            console.log(
              `${index + 1}. ID: ${tx.id}, Fecha: ${tx.date.toLocaleString()}, Tipo: ${tx.type}, Monto: ${tx.totalAmount} coronas, Participante: ${tx.participant.name}, Bienes: ${tx.items.map((item) => item.name).join(", ")}`,
            );
          });
        }
        break;
